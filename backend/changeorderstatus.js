        ...
        tourist.purchasedProducts.forEach((order) => {
          ...
          schedule.scheduleJob(oneMinuteFromNow, async () => {
            try {
              ...
              const refreshedTourist = await touristModel.findById(tourist._id).populate("purchasedProducts.product");
              const updatedOrder = refreshedTourist.purchasedProducts.find(
                (purchased) => purchased.product && purchased.product._id.toString() === order.product._id.toString()
              );
